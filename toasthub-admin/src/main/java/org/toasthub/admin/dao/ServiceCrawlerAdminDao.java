...
import org.toasthub.core.general.dao.ServiceCrawlerDao;
...
@Stateless
public class ServiceCrawlerAdminDao extends ServiceCrawlerDao implements ServiceCrawlerAdminDaoIface {
	...
	public void delete(RestRequest request, RestResponse response) throws Exception {
		if (request.containsParam(BaseEntity.ITEMID) && !"".equals(request.getParam(BaseEntity.ITEMID))) {
			ServiceClass serviceClass = (ServiceClass) entityManagerSvc.getInstance().getReference(ServiceClass.class, new Long((Integer) request.getParam(BaseEntity.ITEMID)));
