...
import org.toasthub.core.preferences.dao.AppOptionDao;

@AppOptionAdminDaoQual
@Stateless
public class AppOptionAdminDao extends AppOptionDao implements AppOptionAdminDaoIface {
	...
	public void save(RestRequest request, RestResponse response) throws Exception {
		...
		entityManagerSvc.getInstance().merge(appPageOptionName);
	}

	@Override
	public void delete(RestRequest request, RestResponse response) throws Exception {
		if (request.containsParam(BaseEntity.ITEMID) && !"".equals(request.getParam(BaseEntity.ITEMID))) {
			AppPageOptionName appPageOptionName = (AppPageOptionName) entityManagerSvc.getInstance().getReference(AppPageOptionName.class, new Long((Integer) request.getParam(BaseEntity.ITEMID)));
			entityManagerSvc.getInstance().remove(appPageOptionName);
