...
import org.toasthub.core.preferences.dao.AppLabelDao;

@AppLabelAdminDaoQual
@Stateless
public class AppLabelAdminDao extends AppLabelDao implements AppLabelAdminDaoIface {
	...
	public void save(RestRequest request, RestResponse response) throws Exception {
		...
		entityManagerSvc.getInstance().merge(appPageLabelName);
	}
	
	@Override
	public void delete(RestRequest request, RestResponse response) throws Exception {
		if (request.containsParam(BaseEntity.ITEMID) && !"".equals(request.getParam(BaseEntity.ITEMID))) {
			AppPageLabelName appPageLabelName = (AppPageLabelName) entityManagerSvc.getInstance().getReference(AppPageLabelName.class, new Long((Integer) request.getParam(BaseEntity.ITEMID)));
			entityManagerSvc.getInstance().remove(appPageLabelName);
