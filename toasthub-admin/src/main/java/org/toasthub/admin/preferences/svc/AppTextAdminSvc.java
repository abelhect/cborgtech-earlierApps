...
import org.toasthub.admin.preferences.dao.AppTextAdminDaoIface;
import org.toasthub.admin.preferences.dao.AppTextAdminDaoQual;
...
import org.toasthub.core.preferences.svc.AppTextSvc;
import org.toasthub.core.security.svc.Authorize;

@AppTextAdminSvcQual
@RequestScoped
public class AppTextAdminSvc extends AppTextSvc implements ServiceProcessor, AppTextAdminSvcIface {

	@Inject @AppTextAdminDaoQual AppTextAdminDaoIface textDao;
	...
	public void delete(RestRequest request, RestResponse response) {
		try {
			textDao.delete(request, response);
			// reset
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			if (request.containsParam(BaseEntity.ITEMID) && !request.getParam(BaseEntity.ITEMID).equals("")) {
				...
				t.setArchive(false);
				t.setLocked(false);
				request.addParam(BaseEntity.ITEM, t);
			}
						
			// marshall
			utilSvc.marshallFields(request, response);
			
			textDao.save(request, response);
			// reset
			appCachePage.clearAppPageTextCache();
