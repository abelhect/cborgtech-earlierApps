...
public class MailDao implements Serializable {
	...
	public void sendMailTLS(String email, String urlParams) throws Exception{
		...
		String serverName = SessionHolder.getURLDomain();
		String contextPath = SessionHolder.getContextpath();
		ExecutorService executor = Executors.newSingleThreadExecutor();
		executor.submit(() -> {
		Session session = Session.getInstance(props,
		  new javax.mail.Authenticator() {
			...
			try {
				...
				message.setText("Welcome new user click link to confirm registration http://"+serverName+":8080/"+contextPath+"/login/login.html?"+urlParams );
				Transport.send(message);
			} catch (Exception e) {
				// need to track this !!
				e.printStackTrace();
