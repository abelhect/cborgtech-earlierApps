		...
		for (var i in items){
			if (items.hasOwnProperty(i)){
				...
				if (items[i].values[0].href != null && items[i].values[0].href != "") {
					...
					href.href = "/".concat(toastHub.contextPath).concat(items[i].values[0].href);
				} else {
				...
				if (items[i].values[0].image != null && items[i].values[0].image != "") {
					...
					if (image.localFile != null) {
						href.innerHTML = "<img src='/".concat(toastHub.contextPath).concat(image.localFile).concat("' alt=''><span class=' fa fa-angle-down'></span>");
